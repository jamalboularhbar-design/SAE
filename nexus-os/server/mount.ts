/**
 * Mount Nexus OS inside a parent Express app (e.g. argbuilder.io at /os).
 */
...
import { z } from "zod";
import { SKILLS, SPECIALISTS, WORKSPACES } from "./catalog.ts";
...
export function createNexusOsApiRouter(): Router {
...
export interface MountOptions {
  /** URL prefix, e.g. `/os` */
  basePath?: string;
  /** Directory with built Vite assets (dist/public) */
  staticDir?: string;
  /** Start heartbeat scheduler (once per process) */
  startScheduler?: boolean;
}

let schedulerStarted = false;

/** Register Nexus OS API + optional static UI on a parent Express app. */
export function registerNexusOs(parent: Express, opts: MountOptions = {}): void {
  const base = (opts.basePath ?? "/os").replace(/\/$/, "");
  const apiPath = `${base}/api`;

  parent.use(apiPath, express.json({ limit: "1mb" }), createNexusOsApiRouter());

  const staticDir = opts.staticDir ?? path.resolve(__dirname, "../dist/public");
  if (fs.existsSync(staticDir)) {
    parent.use(base, express.static(staticDir, { index: "index.html" }));
    parent.get(`${base}/*`, (_req, res) => {
      res.sendFile(path.join(staticDir, "index.html"));
    });
    console.log(`  ⬡ Nexus OS UI → ${base}/`);
  } else {
    console.log(`  ⬡ Nexus OS API → ${apiPath}  (UI static not built — run pnpm build in apps/nexus-os)`);
  }

  if (opts.startScheduler !== false && !schedulerStarted) {
    startScheduler();
