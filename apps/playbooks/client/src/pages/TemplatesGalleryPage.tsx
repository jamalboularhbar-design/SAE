      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">Reusable document templates to jumpstart your writing</p>
            </div>
          </div>
          {isAdmin && (
            <button
        ...
        {showForm && (
          <div className="bg-card border border-border rounded-lg p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <input value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} placeholder="Template name" className="px-3 py-2 bg-input border border-border rounded text-foreground" />
              <input value={formData.category} onChange={e => setFormData(p => ({ ...p, category: e.target.value }))} placeholder="Category" className="px-3 py-2 bg-input border border-border rounded text-foreground" />
              <input value={formData.icon} onChange={e => setFormData(p => ({ ...p, icon: e.target.value }))} placeholder="Icon (emoji)" className="px-3 py-2 bg-input border border-border rounded text-foreground" />
            </div>
            <input value={formData.description} onChange={e => setFormData(p => ({ ...p, description: e.target.value }))} placeholder="Description" className="w-full px-3 py-2 bg-input border border-border rounded text-foreground mb-4" />
            <textarea value={formData.content} onChange={e => setFormData(p => ({ ...p, content: e.target.value }))} placeholder="Template content (Markdown)" rows={8} className="w-full px-3 py-2 bg-input border border-border rounded text-foreground mb-4 font-mono text-sm" />
            <button
        ...
        {previewId && previewTemplate && (
          <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setPreviewId(null)}>
            <div className="bg-card border border-border rounded-lg p-6 max-w-3xl w-full max-h-[80vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold">{previewTemplate.name}</h3>
                <button onClick={() => setPreviewId(null)} className="text-muted-foreground hover:text-foreground"><X className="w-5 h-5" /></button>
              </div>
              <pre className="text-sm text-foreground/80 whitespace-pre-wrap font-mono bg-muted p-4 rounded">{previewTemplate.content}</pre>
        ...
        {categories.map(cat => (
          <div key={cat} className="mb-8">
            <h2 className="text-xl font-semibold text-white mb-4 border-b border-border pb-2">{cat}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {templates?.filter((t: any) => t.category === cat).map((t: any) => (
                <div key={t.id} className="bg-card border border-border rounded-lg p-5 hover:border-[#d4af37]/50 transition-colors">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">Used {t.usageCount} times</span>
                    <div className="flex gap-2">
                      <button onClick={() => setPreviewId(t.id)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
